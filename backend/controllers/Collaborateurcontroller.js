  ...
  try {
    if (req.utilisateur.role !== 'CONTRIBUTEUR') {
      return res.status(400).json({
        message:
          'Seul un contributeur peut demander le statut de collaborateur.',
      });
    }
    ...
    if (!type_organisation || !nom_organisation) {
      return res
        .status(400)
        .json({ message: "Type et nom de l'organisation obligatoires." });
    }

    const dejaEnAttente = await demandeCollaborateurModel.demandeEnAttentePour(
      req.utilisateur.id
    );
    if (dejaEnAttente) {
      return res
        .status(409)
        .json({ message: 'Une demande est déjà en attente de traitement.' });
    }

    const id = await demandeCollaborateurModel.creerDemande(
      req.utilisateur.id,
      {
        type_organisation,
        nom_organisation,
    ...
    try {
      ...
      req.app
        .get('io')
        .to('admins')
        .emit('admin:nouvelle_demande_collaborateur', demande);
    } catch (e) {
      console.error(
        'Erreur emission socket nouvelle_demande_collaborateur:',
        e
      );
    }
...
async function maDemande(req, res) {
  try {
    const demande = await demandeCollaborateurModel.obtenirDerniereDemandePour(
      req.utilisateur.id
    );
...
async function listerSignalements(req, res) {
  try {
    const signalements =
      await signalementModel.listerValidesPourCollaboration();
...
async function proposerSolution(req, res) {
  try {
    ...
    if (!message)
      return res
        .status(400)
        .json({ message: 'Un message de proposition est requis.' });
    ...
    if (signalement.statut !== 'VALIDE') {
      return res.status(400).json({
        message: "Ce signalement n'est pas (ou plus) ouvert aux propositions.",
      });
    }

    const id = await collaborationModel.creerProposition(
      req.params.id,
      req.utilisateur.id,
      message
    );
    ...
    try {
      ...
      req.app
        .get('io')
        .to('admins')
        .emit('admin:nouvelle_proposition', proposition);
    } catch (e) {
...
async function mesPropositions(req, res) {
  try {
    const propositions = await collaborationModel.listerParCollaborateur(
      req.utilisateur.id
    );
...
async function envoyerRapport(req, res) {
  try {
    ...
    if (!rapport)
      return res
        .status(400)
        .json({ message: 'Le rapport ne peut pas être vide.' });
    ...
    if (proposition.collaborateur_id !== req.utilisateur.id) {
      return res
        .status(403)
        .json({ message: "Ce n'est pas votre proposition." });
    }
    if (proposition.statut !== 'ACCEPTEE') {
      return res.status(400).json({
        message:
          "Cette proposition n'a pas (encore) reçu le feu vert de l'administrateur.",
      });
    }

    const misAJour = await collaborationModel.envoyerRapport(
      req.params.id,
      rapport
    );
    ...
    return res.json({ message: "Rapport envoyé à l'administrateur." });
  } catch (err) {

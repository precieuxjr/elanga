...
async function listerDemandes(req, res) {
  try {
    const demandes = await demandeCollaborateurModel.lister({ statut: req.query.statut });
...
async function accepterDemande(req, res) {
  try {
    const demande = await demandeCollaborateurModel.accepter(req.params.id, req.utilisateur.id);
    ...
    req.app.get('io').to(`user_${demande.utilisateur_id}`).emit('collaborateur:demande_traitee', demande);

    return res.json({ message: 'Demande acceptée, utilisateur promu collaborateur.', demande });
  } catch (err) {
...
async function refuserDemande(req, res) {
  try {
    const demande = await demandeCollaborateurModel.refuser(req.params.id, req.utilisateur.id);
    if (!demande) return res.status(404).json({ message: 'Demande introuvable.' });
...
async function listerCollaborations(req, res) {
  try {
    const collaborations = await collaborationModel.lister({ statut: req.query.statut });
...
async function accepterCollaboration(req, res) {
  try {
    const proposition = await collaborationModel.accepter(req.params.id, req.utilisateur.id);
    if (!proposition) return res.status(404).json({ message: 'Proposition introuvable.' });

    req.app.get('io').to(`user_${proposition.collaborateur_id}`).emit('collaborateur:proposition_traitee', proposition);

    return res.json({ message: 'Feu vert donné au collaborateur.', proposition });
  } catch (err) {
...
async function refuserCollaboration(req, res) {
  try {
    const proposition = await collaborationModel.refuser(req.params.id, req.utilisateur.id);
    if (!proposition) return res.status(404).json({ message: 'Proposition introuvable.' });
...
async function cloturerCollaboration(req, res) {
  try {
    ...
    if (proposition.statut !== 'RAPPORT_ENVOYE') {
      return res.status(400).json({ message: 'Aucun rapport à valider pour cette proposition.' });
    }

    const cloturee = await collaborationModel.cloturer(req.params.id, req.utilisateur.id);
    const signalement = await signalementModel.marquerResolu(proposition.signalement_id);

    req.app.get('io').to(`user_${proposition.collaborateur_id}`).emit('collaborateur:proposition_traitee', cloturee);
    ...
    return res.json({ message: 'Collaboration clôturée, signalement marqué résolu.', signalement });
  } catch (err) {
...
async function retirerDeLaCarte(req, res) {
  try {
    const signalement = await signalementModel.definirVisibiliteCarte(req.params.id, false);
    if (!signalement) return res.status(404).json({ message: 'Signalement introuvable.' });

    req.app.get('io').emit('carte:marqueur_retire', { id: signalement.id });

    return res.json({ message: 'Signalement retiré de la carte (toujours en base).', signalement });
  } catch (err) {
...
async function restaurerSurLaCarte(req, res) {
  try {
    const signalement = await signalementModel.definirVisibiliteCarte(req.params.id, true);
    if (!signalement) return res.status(404).json({ message: 'Signalement introuvable.' });
    ...
    const coherenceOk = signalement.commune_coherente === null || signalement.commune_coherente === 1;
    if (signalement.statut === 'VALIDE' && coherenceOk) {
      req.app.get('io').emit('carte:nouveau_marqueur', signalement);
    }

    return res.json({ message: 'Signalement restauré sur la carte.', signalement });
  } catch (err) {

...
<template>
  <div class="max-w-lg mx-auto bg-white rounded-2xl shadow-sm p-8">
    <div class="flex flex-col items-center text-center mb-6">
      <div class="w-14 h-14 rounded-2xl bg-primary-50 text-primary-600 flex items-center justify-center mb-3">
        <HeartHandshake :size="26" />
      </div>
      <h1 class="text-xl font-semibold text-gray-800">Devenir collaborateur</h1>
      <p class="text-sm text-gray-500 mt-1">
        Pour les ONG, écoles et centres environnementaux qui souhaitent intervenir
        directement sur les signalements de terrain.
      </p>
    </div>

    <p v-if="chargement" class="text-center text-sm text-gray-400 py-8">Chargement…</p>

    <!-- Deja collaborateur ou admin : rien a demander -->
    <div v-else-if="dejaCollaborateur" class="bg-primary-50 rounded-xl p-5 text-center">
      <CheckCircle2 :size="24" class="text-primary-600 mx-auto mb-2" />
      <p class="text-sm text-primary-800 font-medium">Vous êtes déjà collaborateur sur ELANGA.</p>
      <p class="text-xs text-primary-600 mt-1">Rendez-vous dans votre espace collaborateur pour intervenir sur les signalements.</p>
    </div>

    <!-- Une demande existe deja et n'est pas encore tranchee -->
    <div v-else-if="demande && demande.statut === 'EN_ATTENTE'" class="bg-orange-50 rounded-xl p-5 text-center">
      <Clock :size="24" class="text-orange-500 mx-auto mb-2" />
      <p class="text-sm text-orange-700 font-medium">Votre demande est en cours d'examen.</p>
      <p class="text-xs text-orange-600 mt-2">
        Organisation : {{ demande.nom_organisation }} ({{ demande.type_organisation }})
      </p>
      <p class="text-xs text-gray-400 mt-1">Envoyée le {{ new Date(demande.date_demande).toLocaleDateString('fr-FR') }}</p>
    </div>

    <!-- Demande deja acceptee mais role pas encore rafraichi cote client -->
    <div v-else-if="demande && demande.statut === 'ACCEPTEE'" class="bg-primary-50 rounded-xl p-5 text-center">
      <CheckCircle2 :size="24" class="text-primary-600 mx-auto mb-2" />
      <p class="text-sm text-primary-800 font-medium">Votre demande a été acceptée !</p>
      <p class="text-xs text-primary-600 mt-1">Reconnectez-vous pour activer votre espace collaborateur.</p>
    </div>

    <!-- Formulaire : pas de demande, ou demande precedente refusee -->
    <template v-else>
      <div v-if="demande?.statut === 'REFUSEE'" class="bg-red-50 rounded-xl p-4 mb-5 text-center">
        <XCircle :size="20" class="text-red-500 mx-auto mb-1" />
        <p class="text-xs text-red-600">
          Votre précédente demande n'a pas été retenue. Vous pouvez en soumettre une nouvelle
      ...
      <form @submit.prevent="soumettre" class="space-y-4">
        <div>
          ...
          <select v-model="formulaire.type_organisation" required
        ...
        <div>
          ...
          <div class="relative mt-1">
            ...
            <input v-model="formulaire.nom_organisation" type="text" required placeholder="Ex : Association Verte Kinshasa"
        ...
        <div>
          <label class="text-sm text-gray-600">Pourquoi voulez-vous rejoindre ELANGA ? (optionnel)</label>
          <textarea v-model="formulaire.justification" rows="3" placeholder="Missions de votre organisation, zones d'intervention habituelles..."
        ...
        <p v-if="succes" class="text-sm text-primary-600">Demande envoyée ! Un administrateur va l'examiner.</p>

...
async function basculerParticipation(evt) {
  ...
  try {
    if (evt.deja_participant)
      await collaborateurService.quitterEvenement(evt.id);
...
function badgeStatut(s) {
  return s === 'ASSAINISSEMENT_EN_COURS'
    ...
    : {
        ...
        label: "En attente d'assainissement",
      };
}
  ...
  <div class="space-y-6 py-6">
    <div class="text-center space-y-2">
      <h1 class="text-3xl font-bold text-gray-900">
        Événements d'assainissement
      </h1>
      <p class="text-gray-500 max-w-lg mx-auto">
        Rejoignez un nettoyage organisé par un collaborateur près de chez vous,
        ou consultez l'impact de ceux déjà réalisés.
      </p>
    </div>
    ...
    <div v-if="ongletActif === 'venir'" class="max-w-3xl mx-auto space-y-4">
      <p
        v-if="chargementEvenements"
        class="text-center text-sm text-gray-400 py-10"
      >
        Chargement…
      </p>
      <p
        v-else-if="!evenements.length"
        class="text-center text-sm text-gray-400 py-10"
      >
      ...
      >
        <div class="flex items-start justify-between gap-3 mb-3">
          <div>
            <span
              :class="badgeStatut(e.statut).classe"
              class="px-2 py-1 rounded-full text-[11px] font-semibold"
              >{{ badgeStatut(e.statut).label }}</span
            >
        ...
        <div class="flex flex-wrap gap-4 text-sm text-gray-500 mb-3">
          <span class="flex items-center gap-1.5"
            ><MapPin :size="14" /> {{ e.commune }} —
            {{ e.type_signalement }}</span
          >
          <span class="flex items-center gap-1.5"
            ><Calendar :size="14" /> {{ formatDate(e.date_evenement) }}</span
          >
        </div>

        <p class="text-sm text-gray-600 bg-gray-50 rounded-xl p-4 mb-4">
          {{ e.message }}
        </p>
        ...
        <button
          @click="basculerParticipation(e)"
          :disabled="enTraitementId === e.id"
          :class="
            e.deja_participant
    ...
    <div v-else class="max-w-4xl mx-auto space-y-6">
      ...
      <p v-if="chargementStats" class="text-center text-sm text-gray-400 py-10">
        Chargement…
      </p>
      <p
        v-else-if="!stats.evenements.length"
        class="text-center text-sm text-gray-400 py-10"
      >
      ...
      <div v-else class="grid md:grid-cols-2 gap-5">
        <div
          v-for="e in stats.evenements"
          :key="e.id"
          class="bg-white rounded-2xl shadow-sm overflow-hidden"
        >
          <div class="grid grid-cols-2">
            <img
              v-if="lienPhoto(e.photo_avant_lien)"
              :src="lienPhoto(e.photo_avant_lien)"
              class="w-full h-36 object-cover"
              alt="Avant"
            />
            <img
              v-if="lienPhoto(e.photo_apres_lien)"
              :src="lienPhoto(e.photo_apres_lien)"
              class="w-full h-36 object-cover"
              alt="Après"
            />
          </div>
          <div class="p-4">
            ...
            <p class="text-xs text-gray-500 mt-1 flex items-center gap-1">
              <MapPin :size="12" /> {{ e.commune }} — {{ e.type_signalement }}
            </p>
            <div
              class="flex items-center justify-between mt-3 text-xs text-gray-400"
            >
              <span class="flex items-center gap-1"
                ><Users :size="12" />
                {{ e.nb_participants }} participant(s)</span
              >
              <span class="flex items-center gap-1"
                ><CheckCircle2 :size="12" class="text-primary-600" />
                {{ new Date(e.date_cloture).toLocaleDateString('fr-FR') }}</span
              >
            </div>
          </div>
        </div>
